...
export default function LoginScreen({ navigation }) {
    ...
    const handleLogin = async () => {
        ...
        if (!identifierErr && !passwordErr) {
            ...
            try {
                const response = await apiService.authenticateLoginCustomer(
                    loginData
                );

                if (response.status === 200) {
                    const { access_token, refresh_token, id, expires_in } =
                        response.data.data;
                    ...
                    ShowAlertWithTitleContentAndOneActions(
                        ...
                        "Chào mừng đến với Computer Store "
                    );
                    ...
                    Alert.alert("Đăng nhập thất bại", errorMessages);
                }
            } catch (error) {
    ...
    const validateIdentifier = (identifier) => {
        ...
        if (emailPattern.test(identifier)) {
            ...
            if (identifier.includes("@")) {
                return "Định dạng email không hợp lệ. Vui lòng nhập địa chỉ email hợp lệ.";
            } else if (/^\d+$/.test(identifier)) {
                return "Định dạng số điện thoại không hợp lệ. Vui lòng nhập số điện thoại hợp lệ.";
            } else {
                return "Định dạng tên người dùng không hợp lệ. Vui lòng nhập tên người dùng hợp lệ.";
            }
        }
    };
                ...
                <View style={styles.container}>
                    <View>
                        ...
                        <Text style={styles.subtitleText}>
                            Chào mừng trở lại với Computer Store
                        </Text>
                    </View>
                    <CustomTextInput
                        value={identifier}
