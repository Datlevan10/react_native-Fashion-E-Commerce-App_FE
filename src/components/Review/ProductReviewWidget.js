...
const ProductReviewWidget = ({ reviews, onWriteReview }) => {
    ...
    const loadMoreReviews = () => {
        ...
        setTimeout(() => {
            ...
            setVisibleCount(
                newCount > reviews.length ? reviews.length : newCount
            );
            setIsLoading(false);
        }, 1000);
    };
    ...
    const onFilterReviews = async (filter, productId) => {
        ...
        try {
            ...
            if (["5", "4", "3", "2", "1"].includes(filter)) {
                response = await apiService.filterReviewsByStar(
                    filter,
                    productId
                );
            } else if (filter === "all") {
                ...
                response = await apiService.filterReviewByMostHelpFul(
                    productId
                );
                    ...
                    <View style={styles.customerInfo}>
                        ...
                        <View style={styles.reviewDateBox}>
                            ...
                            <Text style={styles.reviewDate}>
                                Đánh giá ngày {formatDate(item.review_date)}
                            </Text>
                        </View>
                    </View>
                </View>
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.reviewContainerTitle}>
            ...
            <View style={styles.reviewCard}>
                <View style={styles.statsContainer}>
                    ...
                    <Text style={styles.totalReview}>
                        {reviews[0]?.total_review || 0} Đánh giá
                    </Text>
                    <ScoreBar
                        reviews={reviews}
                        onFilterByStar={(star) =>
                    ...
                    <Text style={styles.titleReviewCard}>
                        Đánh giá sản phẩm này
                    </Text>
                    <Text style={styles.subtitleReviewCard}>
                        Hãy chia sẻ suy nghĩ của bạn với những khách hàng khác.
                    </Text>
                    <TouchableOpacity
                        style={styles.button}
            ...
            <View style={styles.imageReviewContainer}>
                <Text style={styles.title}>Hình ảnh từ các bài đánh giá</Text>
                <TouchableOpacity>
                    <Text style={styles.allPhotos}>Tất cả ảnh</Text>
                </TouchableOpacity>
            </View>
            <ScrollView
                horizontal
            ...
            <Text style={styles.title}>Đánh giá kèm bình luận</Text>
            <ScrollView
                horizontal
                showsHorizontalScrollIndicator={false}
            ...
            >
                {[
                    ...
                    { label: "Hữu ích", filter: "helpful" },
                    { label: "Xếp hạng cao nhất", filter: "highest" },
                    { label: "Xếp hạng thấp nhất", filter: "lowest" },
                    { label: "Oldest", filter: "oldest" },
                    { label: "Mới nhất", filter: "newest" },
                    { label: "Đánh giá kèm ảnh", filter: "media" },
                    { label: "★ 5", filter: "5" },
                    { label: "★ 4", filter: "4" },
                    { label: "★ 3", filter: "3" },
                    { label: "★ 2", filter: "2" },
