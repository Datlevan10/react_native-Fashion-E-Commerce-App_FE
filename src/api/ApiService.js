...
const staffRefreshToken = async (refreshToken) => {
    return api.post("/staffs/auth/refresh-token", {
...
const adminRefreshToken = async (refreshToken) => {
    return api.post("/admins/auth/refresh-token", {
        refresh_token: refreshToken,
    });
};
...
const getOrderStatistics = async () => {
    try {
        ...
        const todayRevenue = orders
            ...
            .reduce(
                (sum, order) =>
                    sum + parseFloat(order.total_price || order.total || 0),
                0
            );
...
const getActiveCarts = async () => {
    try {
        ...
        return {
            data: { data: { active: activeCarts.length, carts: activeCarts } },
        };
    } catch (error) {
...
const createOrder = async (orderData) => {
    console.log("ApiService.createOrder called with:", {
    ...
    try {
        ...
        console.error("Order API error in ApiService:", {
...
const getCustomerReport = async (startDate, endDate) => {
    return api.get(
        `/admin/reports/customers?start_date=${startDate}&end_date=${endDate}`
    );
};

const getProductReport = async (startDate, endDate) => {
    return api.get(
        `/admin/reports/products?start_date=${startDate}&end_date=${endDate}`
    );
};
    ...
    try {
        ...
        return {
            data: {
                ...
                product_count: (response.data.data || response.data || [])
                    .length,
            },
        };
    } catch (error) {
